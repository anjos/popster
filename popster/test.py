...
def test_make_dirs():
    ...
    with TemporaryDirectory() as base:
        ...
        mode = (
            stat.S_IFDIR | stat.S_ISGID | stat.S_IRWXU | stat.S_IRWXG
        )  # drwxrws---
...
def test_move_dry():
    ...
    with TemporaryDirectory() as base, TemporaryDirectory() as dst:
        ...
        src = os.path.join(subfolder, os.path.basename(src))
        result = copy(
            src, dst, fmt, timestamp=False, nodate="nodate", move=True, dry=True
        )
...
def test_watch_move():
    ...
    with TemporaryDirectory() as start, TemporaryDirectory() as base, TemporaryDirectory() as dst:
